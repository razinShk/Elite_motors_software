  ...
  useEffect(() => {
    ...
    const searchDatabase = async () => {
      try {
        ...
        // Search vehicles
        const vehiclesTbl = getTableName('vehicles');
        const customersTbl = getTableName('customers');

        const { data: vehicles } = await supabase
          .from(vehiclesTbl)
          .select(`
            *,
            customers:${customersTbl} (name)
          `)
          .or(`make.ilike.%${query}%,model.ilike.%${query}%,registration_number.ilike.%${query}%`)
          .limit(5);

        // Search services
        const servicesTbl = getTableName('services');

        const { data: services } = await supabase
          .from(servicesTbl)
          .select(`
            *,
            vehicles:${vehiclesTbl} (
              make,
              model,
              customers:${customersTbl} (name)
            )
