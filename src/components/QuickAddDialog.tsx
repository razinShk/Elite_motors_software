...
export const QuickAddDialog = () => {
  ...
  const handleAddPart = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const { error } = await supabase.from(getTableName('spare_parts')).insert({
  ...
  const handleAddCustomer = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const { error } = await supabase.from(getTableName('customers')).insert({
  ...
  const handleAddServiceType = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const { error } = await supabase.from(getTableName('service_types')).insert({
        name: formData.get('name') as string,
