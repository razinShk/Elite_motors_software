...
export const useReportsData = () => {
  ...
  return useQuery({
    queryKey: ['reports_data', getTableName('sales')],
    queryFn: async () => {
      ...
      const { data: saleItems } = await supabase
        ...
        .select(`
          quantity,
          spare_parts:${spare_parts}(part_name),
          sales:${getTableName('sales')}(sale_date)
        `)
      ...
      const { data: serviceParts } = await supabase
        ...
        .select(`
          quantity,
          spare_parts:${spare_parts}(part_name),
          services:${services}(service_date)
        `)
      ...
      const { data: topCustomersData } = await supabase
        .from(services)
        .select(`
          vehicles:${getTableName('vehicles')}(customers:${getTableName('customers')}(id, name)),
          total_cost,
          service_date
        `)
