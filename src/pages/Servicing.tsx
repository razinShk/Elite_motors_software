...
import { useServicesData, useServiceTypes, useCreateCustomerAndVehicle, useCreateService, useCustomersData, useDeleteService } from '@/hooks/useServicesData';
import { useSparePartsData } from '@/hooks/useSparePartsData';
import { useToast } from '@/hooks/use-toast';
import { ServiceInvoice } from '@/components/ServiceInvoice';
import { ManageServiceTypesButton } from '@/components/ManageServiceTypesButton';
...
const Servicing = () => {
  ...
  const selectedServiceType = serviceTypes?.find(t => t.id === newService.service_type_id);
  const serviceTypeBase = Number(selectedServiceType?.base_price || 0);
  const laborAmount = parseFloat(newService.labor_charges || '0');
  const partsTotal = getTotalPartsAmount();
  const preDiscountSubtotal = serviceTypeBase + laborAmount + partsTotal;
  const discountValueNum = parseFloat(newService.discount_value || '0');
  const computedDiscount = newService.discount_type === 'percent'
    ? (preDiscountSubtotal * (discountValueNum || 0)) / 100
    : (discountValueNum || 0);
  const finalTotal = Math.max(0, preDiscountSubtotal - computedDiscount);
          ...
          <Dialog open={isCreateServiceOpen} onOpenChange={setIsCreateServiceOpen}>
            <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
              ...
              {serviceStep === 1 && (
                <div className="space-y-6">
                  <div>
                    <Label htmlFor="select_vehicle">Select Customer & Vehicle</Label>
                    <div className="space-y-2">
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue placeholder="Select a customer and vehicle for service" />
                        </SelectTrigger>
                        <SelectContent className="max-h-60">
                          {customers?.filter(customer => 
                            !vehicleSearchTerm || 
                            customer.name.toLowerCase().includes(vehicleSearchTerm.toLowerCase()) ||
                            customer.vehicles?.some(vehicle => 
                              `${vehicle.make} ${vehicle.model} ${vehicle.registration_number}`.toLowerCase().includes(vehicleSearchTerm.toLowerCase())
                            )
                          ).map((customer) => 
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <select
                          ...
                          onChange={(e) => setNewService({ ...newService, discount_type: e.target.value as any })}
                        >
                        ...
                        <Input
                          type="number"
                          step="0.01"
                          placeholder={newService.discount_type === 'percent' ? '10' : '250'}
                          value={newService.discount_value}
                          onChange={(e) => setNewService({ ...newService, discount_value: e.target.value })}
                        />
                        <Input
                          placeholder="Name (optional)"
                          value={newService.discount_name}
                          onChange={(e) => setNewService({ ...newService, discount_name: e.target.value })}
                        />
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                        <div className="space-y-2">
                          <div className="flex justify-between">
                            ...
                            <span>₹{serviceTypeBase.toFixed(2)}</span>
                          </div>
                          <div className="flex justify-between">
                          ...
                          {(discountValueNum || 0) > 0 && (
                            <div className="flex justify-between text-red-600">
                              <span>Discount{newService.discount_name ? ` - ${newService.discount_name}` : ''} ({newService.discount_type === 'percent' ? `${discountValueNum}%` : `₹${discountValueNum}`}):</span>
              ...
              <div key={service.id} className="border rounded-lg p-4">
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center">
                  ...
                  <div className="flex justify-end gap-2">
                    <Button variant="destructive" size="sm" disabled={deleteServiceMutation.isPending} onClick={() => deleteServiceMutation.mutate(service.id)}>
